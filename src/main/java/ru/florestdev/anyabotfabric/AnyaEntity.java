...
public class AnyaEntity extends PathAwareEntity {
    ...
    protected AnyaEntity(EntityType<? extends PathAwareEntity> type, World world) {
        super(type, world);
        setInvulnerable(true);
        profile = new GameProfile(ANYA_UUID, "AnyaChan");
    ...
    public void tick() {
        ...
        if (!getWorld().isClient) {
            ...
            if (Anyabotfabric.isFollowPlayerEnabled()) {
                PlayerEntity p = null;
                for (var entry : Anyabotfabric.playerToAnya.entrySet()) {
                ...
                if (getWorld().getTimeOfDay() >= 13000 && getWorld().getTimeOfDay() <= 23000) {
                    if (isSleeping()) {
                        ...
                        if (!warned) {
                            if (p != null ) {
                                p.sendMessage(Text.of("<Anya> %s, я хочу спать! :(".formatted(p.getName().getString())), false);
                            }
                            warned = true;
                        }
                    }
