...
public class KiraEntity extends PathAwareEntity {
    ...
    protected KiraEntity(EntityType<? extends PathAwareEntity> type, World world) {
        super(type, world);
    ...
    public void tick() {
        ...
        if (!getWorld().isClient) {
            ...
            if (Anyabotfabric.isFollowPlayerEnabled()) {
                PlayerEntity p = null;
                for (var entry : Anyabotfabric.playerToKira.entrySet()) {
                ...
                if (getWorld().getTimeOfDay() >= 13000 && getWorld().getTimeOfDay() <= 23000) {
                    if (isSleeping()) {
                        ...
                        if (!warned) {
                            if (p != null ) {
                                p.sendMessage(Text.of("<Kira> %s, я хочу спать! :(".formatted(p.getName().getString())), false);
                            }
                            warned = true;
                        }
                    }
